...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in data_loader:
        ...
        losses_sum = sum(loss_dict[k] for k in loss_dict.keys() if k in weight_dict)
        # increase their own weight
        weight=torch.tensor(0.0001,device=device)
        # for k in loss_dict.keys():
        #     weight_dict[k]+=weight*(loss_dict[k]/losses_sum)

        losses = sum((loss_dict[k]+weight*(1-weight_dict[k]/losses_sum)) * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)
